...
function get_blob_path(blob) {
  var blob_path = blob.key.substring(0, 2) + '/' + blob.key.substring(2);
  return blob_path;
}
...
Repo.prototype.init_from_dir = function(options, callback) {
  ...
  check_directories(root_dir, function(err) {
    if(err) {
      return callback(new Error('fatal: Not a git repository'));
    } else {
      self.root_dir = root_dir;
      return callback();
    }
...
Repo.prototype.write_blob = function(blob, callback) {
  var blob_path = get_blob_path(blob);
